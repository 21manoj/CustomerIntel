...
- `admin` — every account **of its own tenant**, plus Users/Settings (invite, deactivate, change role, playbook + webhook config). A tenant's administrator, never the platform's.
- `cro` / `cfo` / `csm` — the same tenant scope, fewer powers within it; and, within a tenant, `allowed_account_ids` (NULL = all accounts).
- **Tenant scope comes from `allowed_customer_ids` only, for every role, and is fail-closed** — NULL or `[]` reaches nothing, not even the user's own tenant. Both creation paths (`create_customer`'s admin user, `users.invite`'s any-role user) write it; revision `0005_user_tenant_scope_backfill` filled in every row that predates them.

> **Corrected 2026-09-08 — this section originally said the opposite, and shipped a live cross-tenant leak.** It read "NULL = every tenant the platform key can see, for a single-tenant deployment this is moot", and `admin` was written as "every tenant". The deployment was never single-tenant, nothing ever populated `allowed_customer_ids`, and `create_customer` gives every new tenant's *own* first user `role='admin'` — so every logged-in user of every tenant could read and act on any other tenant by passing its `customer_id`, up to minting a password-setup token for another tenant's admin. The lesson kept in the code: a role says *what* you may do, a scope says *whose* data you may do it to, and a missing scope is a provisioning gap, never a grant. The only platform-wide identity is the Bearer `MCP_SERVER_API_KEY` (`mcp_server/auth.py`) — a separate mechanism, correct as it stands, and deliberately not reachable by session cookie. If a session-cookie platform-operator role is ever genuinely needed, it must be its own named thing, not an overload of tenant `admin`.
- A decorator `auth.require_session(role=None)` on every `/app/api/*` route: 401 with no valid session, 403 if `role` is given and doesn't match; the route handler itself applies the account/customer scoping by filtering its query through `current_user().allowed_customer_ids/allowed_account_ids`, the same shape `CustomerApiKey.has_account_access` already does for keys. **The role gate is not the tenant check** — every route naming a `customer_id` also runs it through `http._scoped_cid`/`auth.allows_customer`, and `tests/test_app_api_tenant_isolation.py` fails if a route in `http.ROUTES` has neither an isolation test nor a written exemption. Four admin-only routes (calibrations, playbooks/config, users list, users invite/patch/reset) originally had the role gate and no tenant check at all.
...
### 5a. How it is actually served — settled 2026-09-08

Until this change the frontend only ever ran under `npm run dev` on a laptop: the deployed box served the app's bare JSON descriptor at `/` and nothing else, `backend/server.py` had no static-file serving, the Docker build context was `backend/`-only so `frontend/` never entered any image, and the Caddy site block had no route for it. It is now served at the root of the **existing** hostname, `https://customerintelv1.3-218-251-181.sslip.io/`, with every API path unchanged. No new hostname, no DNS change.

- **Caddy, not `StaticFiles` in `server.py`.** The box already has Caddy in front; serving bundles from the uvicorn process would have put routing in two places.
- **`frontend/` builds itself into its own image.** `frontend/Dockerfile`: `node:22-alpine` runs `npm ci && npm run build`, then `caddy:2-alpine` serves the resulting `dist/` from `/srv` using `frontend/Caddyfile`. It runs as `customerintelv1-web` in the same compose project. The EC2 box needs no Node, and `docker compose up -d --build` — already the one command `deploy_ec2.sh` runs — now covers the frontend too, so there is no separate build step to forget. A TypeScript error fails the image build and so fails the deploy.
- **The shared Caddy splits the hostname by explicit API prefix** (`deploy/Caddyfile.customerintelv1.snippet`): `/mcp`, `/health`, `/api/*`, `/app/api/*` go straight to `customerintelv1-app:8101` exactly as before; everything else goes to `customerintelv1-web:8080`, which serves the static files and falls back to `index.html` so client-side routes (`/portfolio`, `/accounts/64/canvas`) survive a reload or a deep link. Keeping the API arm pointed at the app, rather than chaining it through the web container, means the new container is never in the path of MCP or the API — if it is down or mid-rebuild only the browser app is affected.
- **That prefix list is checked, not trusted.** `backend/scripts/check_caddy_routes.py` enumerates routes off the real ASGI app, parses the prefixes out of the real snippet, and fails the deploy if a registered route is uncovered. The failure it prevents is silent: an uncovered `/api/...` route would be answered by the SPA with HTTP 200 and an HTML body, not a 404.
- **`deploy_ec2.sh` now rewrites the Caddy site block on every deploy** instead of appending it once. The old `if ! grep -q hostname` meant a snippet change never reached an already-deployed box — this change would have silently not happened. The rewrite is idempotent and preserves unrelated sites and comments; the previous file is kept at `~/caddy/Caddyfile.bak` and restored if `caddy validate` rejects the result.
- **`GET /` on the app — the JSON descriptor `{"server":…,"mcp":"/mcp"}` — is now shadowed by the SPA.** Deliberate: it was a placeholder for exactly this, and `/health` carries the same fields. It is listed in `check_caddy_routes.py` as an intentional shadow so it is not mistaken for drift later.
- **This adds no new authentication surface.** Every `/app/api/*` route already required a session cookie and still does; what changes is that the login page is reachable from a browser instead of only from a developer's laptop. Self-service signup remains explicitly not built (§2, §6) — an admin still relays a one-time setup link out of band.
